...
def render_monthly_trend(df, unit, prefix):
    ...
    # [수정포인트] 좌측 상단에 구성비 표기 버튼, 우측에 연도 선택 배치
        ...
        if not ts_df.empty:
            ...
            for grp in GROUP_ORDER:
                if grp in ts_ratio.columns:
                    ...
                    # [수정포인트] 투명도 제거 (fillcolor 색상을 고유색상으로 100% 진하게 채움)
                    fig_ts.add_trace(go.Scatter(
                        ...
                        textfont=dict(size=18, color="#111") # [수정포인트] 글씨 크기 20% 추가 확대
                    ))
            
            # [수정포인트] 활성화 시 3, 6, 9, 12월에 해당하는 위치에 세로 점선 추가
            if show_ts_ratio:
                for m_str in ts_ratio.index:
                    if int(m_str.split('.')[1]) in [3, 6, 9, 12]:
                        fig_ts.add_vline(x=m_str, line_width=1.5, line_dash="dash", line_color="rgba(100, 100, 100, 0.7)", layer="below")
            ...
            fig_ts.update_layout(
                xaxis=dict(
                    ...
                    tickangle=-45,
                    dtick=1 # [수정포인트] 데이터가 생략되지 않고 모두 표기되도록 고정
                )
            )
    ...
    for grp in GROUP_ORDER:
        ...
        # 순수 Bar Trace만 추가 (text는 주석으로 따로 처리)
    ...
    # [수정포인트] 주석(Annotation)을 활용하여 비율이 낮으면 화살표와 함께 우측으로 빼서 표기
    stack_annotations = []
        ...
        for grp in GROUP_ORDER:
            ...
            if val >= 1.0:
                ...
                if val >= 7.0:
                    ...
                    # 좁은 칸(업무용 등)은 화살표 지시선을 사용해 우측으로 도출
                    stack_annotations.append(dict(
                        x=label, y=mid_y, xref='x', yref='y',
                        text=f"{val:.1f}%", xanchor='left',
                        ax=35, ay=0, showarrow=True, arrowhead=0, arrowcolor="#555",
