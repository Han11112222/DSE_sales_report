...
def render_monthly_trend(df, unit, prefix):
                ...
                if grp in ts_ratio.columns:
                    ...
                    fig_ts.add_trace(go.Scatter(
                        ...
                        fillcolor=f"rgba({tuple(int(COLOR_MAP.get(grp, '#000')[i:i+2], 16) for i in (1, 3, 5))[0]}, "
                                  ...
                                  f"{tuple(int(COLOR_MAP.get(grp, '#000')[i:i+2], 16) for i in (1, 3, 5))[2]}, 0.9)",
                        text=text_arr,
                        textposition='top center',
                        textfont=dict(size=15, color="#111")  # [수정] 텍스트 크기 20% 상향 (15pt)
                    ))
            ...
            if show_ts_ratio:
                for m_str in ts_ratio.index:
                    if int(m_str.split('.')[1]) in [3, 6, 9, 12]:
                        fig_ts.add_vline(x=m_str, line_width=1.2, line_dash="dash", line_color="rgba(100, 100, 100, 0.5)", layer="below")
    ...
    for grp in GROUP_ORDER:
        ...
        # [수정포인트] 스택그래프 텍스트 크기를 범례/축 수준(14pt)으로 조정 & 위치 auto 처리
