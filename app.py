...
def render_monthly_trend(df, unit, prefix):
            ...
            for grp in GROUP_ORDER:
                if grp in ts_ratio.columns:
                    ...
                    fig_ts.add_trace(go.Scatter(
                        ...
                        textfont=dict(size=22, color="#111") # [수정] 텍스트 크기 20% 추가 확대
                    ))
            
            # [수정] add_vline 대신 Scatter를 사용하여 축 붕괴 에러 원천 차단 + 세로 점선 추가
            if show_ts_ratio:
    ...
    for grp in GROUP_ORDER:
        ...
        # [수정] 이중 텍스트 에러 및 지시선 제거를 위해 bar trace의 text 속성을 완전히 뺌
    ...
    # [수정] 주석을 활용하여 모든 텍스트를 동일한 큰 사이즈(23pt)로 통일하고 칸이 좁으면 우측으로 빼기
    stack_annotations = []
        ...
        for grp in GROUP_ORDER:
            ...
            if val >= 1.0:
                ...
                if val >= 8.0:
                    ...
                    # 좁은 칸은 우측 바깥으로 도출 (선 없음, 깔끔하게 텍스트만)
                    stack_annotations.append(dict(
                        x=label, y=mid_y, xref='x', yref='y',
        ...
        if not selected_groups:
            ...
            components.html("""
                ...
                @media print {
                    ...
                    .main .block-container, .block-container { padding-top: 0 !important; max-width: 100% !important; width: 100% !important; margin: 0 auto !important; }
                    [data-testid="stAppViewContainer"] > section:nth-child(2) { padding-top: 0 !important; max-width: 100% !important; width: 100% !important; }
