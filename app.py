...
# 그룹 순서 및 기존 사진과 동일한 색상 맵핑 (수정 완료)
GROUP_ORDER = ["가정용", "산업용", "업무용", "영업용", "기타"]
COLOR_MAP = {
    "가정용": "#1F77B4",  # 기존 사진의 진한 파랑
    "산업용": "#AEC7E8",  # 기존 사진의 연한 하늘색
    "업무용": "#D62728",  # 기존 사진의 빨강
    "영업용": "#FF7F0E",  # 기존 사진의 주황
    "기타": "#2CA02C"     # 기존 사진의 초록
}
...
# ─────────────────────────────────────────────────────────
# 그래프 섹션 1: 월별 추이 (2026년 4~12월 계획량 점선 반영)
# ─────────────────────────────────────────────────────────
def render_monthly_trend(df, unit, prefix):
    ...
    with c2: sel_month = st.selectbox("실적 마감 월 (2026년 기준)", options=list(range(1, 13)), index=2, key=f"{prefix}m") # 기본 3월
    with c3: st.markdown("<div style='padding-top:28px;font-size:14px;color:#666;'>집계 기준: <b>단월 판매량</b></div>", unsafe_allow_html=True)

    # 용도 버튼 복구
    try:
        sel_group = st.segmented_control("용도 선택", options=["총량"] + GROUP_ORDER, selection_mode="single", default="총량", key=f"{prefix}sg")
    except:
        sel_group = st.radio("용도 선택", options=["총량"] + GROUP_ORDER, index=0, horizontal=True, key=f"{prefix}rd")

    plot_df = df[df["그룹"] == sel_group] if sel_group != "총량" else df
    
    fig = go.Figure()
    # 연도별 색상 (2026년만 진하게 강조)
    colors = {"2022": "#E2E8F0", "2023": "#CBD5E0", "2024": "#A0AEC0", "2025": "#718096", "2026": "#1F77B4"}
    ...
    for year in range(2022, 2027):
        ...
        # 2026년은 사용자가 선택한 '실적 마감 월'까지만 실적선으로 그림
        if year == 2026:
            y_act = y_act[y_act["월"] <= sel_month]
        ...
        # [2026년 계획 부분 - 점선 노출]
        if year == 2026:
            # 실적 마감 월(4월~12월) 데이터를 계획 시트에서 가져옴
            ...
            if not y26_plan.empty:
                ...
                fig.add_trace(go.Scatter(x=y26_plan["월"], y=y26_plan["값"], mode='lines+markers', 
                                         name="2026년 계획(4~12월)", line=dict(color=colors["2026"], width=3, dash='dot')))
...
# ─────────────────────────────────────────────────────────
# 그래프 섹션 2: 기간별 용도 누적 실적 (스택 색상 정밀 수정)
# ─────────────────────────────────────────────────────────
def render_stacked_chart(df, unit, prefix):
    st.markdown("---")
    st.markdown("### 🧱 기간별 용도 누적 실적 (실적 기준)")
    
    c1, c2 = st.columns([2, 2])
    with c1: plot_years = st.multiselect("연도 선택", options=range(2022, 2027), default=list(range(2022, 2027)), key=f"{prefix}stk_y")
    with c2: period = st.radio("집계 기간", ["연간", "상반기(1~6월)", "하반기(7~12월)"], horizontal=True, key=f"{prefix}stk_p")
    ...
    fig.add_trace(go.Scatter(x=total_line["연"], y=total_line["값"], mode='lines+markers+text', 
                             name="총합계", text=total_line["값"].apply(lambda x: f"{x:,.0f}"),
